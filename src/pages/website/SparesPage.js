    ...
    <div style={{ minHeight: '100vh', background: '#F8FAFC' }}>
      ...
      <div className="pw-page-hero" style={{ background: 'linear-gradient(135deg,#0F9D58,#065F46)', padding: '60px 0 40px', textAlign: 'center' }}>
        <div className="site-max">
          <span className="section-tag" style={{ background: 'rgba(127,232,197,.2)', color: '#a7f3d0' }}>Spare Parts</span>
      ...
      <div className="site-max" style={{ padding: '36px 20px 80px' }}>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 32, justifyContent: 'center' }}>
          {CATS.map(c => (
            <button key={c} onClick={() => setCat(c)} style={{
              ...
              background: cat === c ? '#0F9D58' : '#fff',
              color: cat === c ? '#fff' : '#64748B',
              borderColor: cat === c ? '#0F9D58' : '#e2e8f0',
              ...
              >
                ...
                <div style={{ height: 180, overflow: 'hidden', position: 'relative', background: '#f8fafc' }}>
                  ...
                  {item.brand && item.brand !== 'GENERIC' && (
                    <div style={{ position: 'absolute', top: 10, right: 10, padding: '4px 10px', borderRadius: 999, fontSize: 10, fontWeight: 700, background: 'rgba(0,155,0,.85)', color: '#a7f3d0' }}>
                      {item.brand}
                    </div>
                  )}
                </div>
                ...
                <div style={{ padding: '16px' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 11, fontWeight: 600 }}>
                      <div style={{ width: 8, height: 8, borderRadius: '50%', background: item.currentStock > 10 ? '#0F9D58' : item.currentStock > 0 ? '#F59E0B' : '#EF4444' }} />
                  ...
                  <a href="tel:09952828740" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, width: '100%', padding: '9px', borderRadius: 12, background: '#0F9D58', color: '#fff', fontSize: 12, fontWeight: 700, textDecoration: 'none', transition: 'all .2s' }}
        ...
        {filtered.length === 0 && !loading && (
          <div style={{ textAlign: 'center', padding: '60px 20px' }}>
            ...
            <button onClick={() => { setSearch(''); setCat('ALL'); }} style={{ marginTop: 16, padding: '10px 24px', borderRadius: 12, background: '#0F9D58', color: '#fff', border: 'none', cursor: 'pointer', fontWeight: 600 }}>Clear filters</button>
          </div>
        )}
      </div>
    </div>
