...
export default function GalleryPage() {
  ...
  return (
    <div>
      {/* Header */}
      <div style={{ background:'linear-gradient(135deg,#009B00,#007A00)', padding:'clamp(40px,5vw,60px) 0 clamp(32px,4vw,44px)' }}>
        <div className="site-max" style={{ textAlign:'center' }}>
          <div style={{ display:'inline-flex', alignItems:'center', gap:8, background:'rgba(255,255,255,.1)', backdropFilter:'blur(8px)', border:'1px solid rgba(255,255,255,.2)', color:'#7fff7f', fontSize:12, fontWeight:700, padding:'6px 14px', borderRadius:20, marginBottom:16, letterSpacing:'1px', textTransform:'uppercase' }}>
      ...
      <div className="site-max" style={{ padding:'clamp(24px,3vw,40px) clamp(12px,4vw,32px)' }}>
              ...
              {filtered.map((item, i) => (
                ...
                >
                  ...
                  <div style={{ position:'absolute', top:8, left:8 }}>
                    <span style={{ background:'rgba(0,155,0,.85)', backdropFilter:'blur(4px)', color:'#7fff7f', fontSize:9, fontWeight:700, padding:'2px 8px', borderRadius:10 }}>{item.category}</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        {/* CTA */}
        <div style={{ textAlign:'center', marginTop:48, background:'linear-gradient(135deg,#009B00,#007A00)', borderRadius:20, padding:'clamp(28px,4vw,44px)' }}>
          ...
          <div style={{ display:'flex', gap:10, justifyContent:'center', flexWrap:'wrap' }}>
            <button onClick={()=>setEnquireOpen(true)} style={{ display:'inline-flex', alignItems:'center', gap:7, padding:'12px 22px', background:'#fff', color:'#009B00', fontWeight:800, fontSize:14, borderRadius:10, border:'none', cursor:'pointer', transition:'all .2s' }}
