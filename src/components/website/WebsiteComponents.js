...
export function PriceDisplay({ price, originalPrice, pricingMode, size = 'md', onEnquire }) {
    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
        <button
          ...
          style={{
            background: 'linear-gradient(135deg,#d4821a,#e8951f)', color: '#fff',
        ...
        <span style={{ fontSize: 10, background: '#e0f9e0', color: '#009B00', padding: '2px 8px', borderRadius: 10, fontWeight: 700 }}>Best Price</span>
      </div>
    );
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, flexWrap: 'wrap' }}>
      <span style={{ fontSize: fs, fontWeight: 900, color: '#009B00', letterSpacing: '-0.5px' }}>
    ...
    >
      ...
      <div className="product-img" style={{ position: 'relative', height: 200 }}>
        ...
        <div style={{ position: 'absolute', top: 10, left: 10 }}>
          <span style={{ background: 'rgba(0,155,0,.85)', backdropFilter: 'blur(6px)', color: '#7fff7f', fontSize: 10, fontWeight: 700, padding: '3px 9px', borderRadius: 20, letterSpacing: '0.5px' }}>
        ...
        {!isEnquireOnly && savings > 0 && (
          <div style={{ position: 'absolute', top: 10, right: 10 }}>
            <span style={{ background: 'linear-gradient(135deg,#d4821a,#e8951f)', color: '#fff', fontSize: 11, fontWeight: 700, padding: '4px 9px', borderRadius: 20 }}>
              {savings}% OFF
            </span>
          </div>
        )}
      ...
      <div style={{ padding: '18px 16px', flex: 1, display: 'flex', flexDirection: 'column', gap: 10 }}>
        ...
        {isFree ? (
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <span style={{ fontSize: 13, fontWeight: 700, color: '#d4821a', fontStyle: 'italic' }}>Enquire for Best Price</span>
        ...
        <button
          ...
          style={{
            ...
            background: isEnquireOnly && !isFree
              ? 'linear-gradient(135deg,#d4821a,#e8951f)'
              : 'linear-gradient(135deg,#009B00,#007A00)',
...
const BRAND_DATA = {
  ...
  'Aqua Fresh':   { bg:'linear-gradient(145deg,#0076a3,#0891b2)', logo:'Aqua Fresh',    tagline:'Pure & Fresh Water',   badge:'🌿', accent:'#7fff7f', logoFont:"'Arial',sans-serif",       logoSize:18, logoColor:'#fff', logoSpacing:'0px' },
...
function FloatingBrandCard({ brand, index }) {
  ...
  const d = BRAND_DATA[brand.name] || { bg:'linear-gradient(145deg,#009B00,#062e23)', logo:brand.name, tagline:'Water Purifier', badge:'💧', accent:'#7fff7f', logoFont:"Arial", logoSize:16, logoColor:'#fff', logoSpacing:'0px' };
    ...
    <footer style={{ background: 'linear-gradient(180deg,#0a2e1e 0%,#061a10 100%)', color: '#fff', paddingTop: 'clamp(40px,6vw,72px)' }}>
      <div className="container">
        <div className="footer-grid">
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 18 }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 10, color: '#7fff7f', letterSpacing: '1.5px', textTransform: 'uppercase', marginTop: 2 }}>RO Water · Coimbatore</div>
              </div>
            </div>
          ...
          {/* Quick Links */}
          <div>
            <div style={{ fontSize: 13, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '1.5px', color: '#7fff7f', marginBottom: 18 }}>Quick Links</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {QUICK.map(q => (
                <Link key={q.to} to={q.to} style={{ display: 'flex', alignItems: 'center', gap: 7, fontSize: 13, color: 'rgba(255,255,255,.65)', textDecoration: 'none', transition: 'all .15s' }}
                  onMouseEnter={e => { e.currentTarget.style.color = '#7fff7f'; e.currentTarget.style.paddingLeft = '4px'; }}
                  onMouseLeave={e => { e.currentTarget.style.color = 'rgba(255,255,255,.65)'; e.currentTarget.style.paddingLeft = '0'; }}>
                  <ChevronRight size={13} color="#7fff7f" />
                  {q.label}
                </Link>
              ))}
            </div>
          ...
          {/* Services */}
          <div>
            <div style={{ fontSize: 13, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '1.5px', color: '#7fff7f', marginBottom: 18 }}>Our Services</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {SERVICES.map(s => (
                <div key={s} style={{ display: 'flex', alignItems: 'center', gap: 7, fontSize: 13, color: 'rgba(255,255,255,.65)' }}>
                  <Droplets size={12} color="#7fff7f" style={{ flexShrink: 0 }} />
                  {s}
                </div>
              ))}
            </div>
          ...
          {/* Contact */}
          <div>
            <div style={{ fontSize: 13, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '1.5px', color: '#7fff7f', marginBottom: 18 }}>Contact Info</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              {[
                ...
                <div key={i} style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                  <div style={{ width: 30, height: 30, borderRadius: 8, background: 'rgba(127,232,197,.1)', border: '1px solid rgba(127,232,197,.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#7fff7f', flexShrink: 0 }}>
                    {c.icon}
                  </div>
                  <div>
                    {c.lines.map((l, j) => c.link
                      ? <a key={j} href={c.link} style={{ display: 'block', fontSize: 13, color: 'rgba(255,255,255,.7)', textDecoration: 'none', lineHeight: 1.6 }} onMouseEnter={e => e.target.style.color = '#7fff7f'} onMouseLeave={e => e.target.style.color = 'rgba(255,255,255,.7)'}>{l}</a>
          ...
          <div style={{ display: 'flex', gap: 20 }}>
            {['Privacy Policy', 'Terms of Use', 'Sitemap'].map(t => (
              <a key={t} href="#" style={{ fontSize: 11, color: 'rgba(255,255,255,.35)', textDecoration: 'none', transition: 'color .15s' }}
                onMouseEnter={e => e.target.style.color = '#7fff7f'}
...
export function AnimatedCounter({ end, suffix = '', prefix = '', label, icon }) {
  ...
  return (
    <div ref={ref} style={{ textAlign: 'center', padding: '16px 8px' }}>
      <div style={{ fontSize: 'clamp(28px,4vw,44px)', fontWeight: 900, color: '#7fff7f', fontFamily: "'Playfair Display',serif", lineHeight: 1, letterSpacing: '-1px' }}>
