  ...
  return (
    <>
      ...
      <nav style={{
        ...
        <div className="container" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 66, gap: 16 }}>
          ...
          <Link to="/" style={{ display: 'flex', alignItems: 'center', gap: 11, flexShrink: 0 }}>
            <div style={{ position: 'relative' }}>
              ...
              <div style={{ position: 'absolute', bottom: -2, right: -2, background: '#7fff7f', width: 12, height: 12, borderRadius: '50%', border: '2px solid #009B00' }} />
            </div>
            <div>
          ...
          <div style={{ display: 'flex', gap: 2, alignItems: 'center' }} className="hide-mobile">
            {NAV_LINKS.map(link => (
              <Link key={link.to} to={link.to} style={{
                ...
                borderBottom: isActive(link.to) ? '2px solid #7fff7f' : '2px solid transparent',
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexShrink: 0 }} className="hide-mobile">
            ...
            <button onClick={onQuote} style={{ display: 'flex', alignItems: 'center', gap: 6, color: '#fff', fontSize: 13, fontWeight: 700, padding: '9px 17px', borderRadius: 8, background: 'linear-gradient(135deg,#d4821a,#e8951f)', border: 'none', cursor: 'pointer', boxShadow: '0 2px 10px rgba(212,130,26,.4)' }}>
        ...
        {/* Mobile menu */}
        {mobileOpen && (
          <div style={{ background: '#009B00', borderTop: '1px solid rgba(255,255,255,.1)', padding: '12px 0' }} className="show-mobile">
            {NAV_LINKS.map(link => (
              <Link key={link.to} to={link.to} style={{ display: 'block', color: isActive(link.to) ? '#7fff7f' : 'rgba(255,255,255,.8)', padding: '12px 24px', fontWeight: isActive(link.to) ? 700 : 500, fontSize: 15, borderLeft: isActive(link.to) ? '3px solid #7fff7f' : '3px solid transparent' }}>
                {link.label}
              </Link>
            ))}
